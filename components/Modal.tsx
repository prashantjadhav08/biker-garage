...
export default function Modal({ isOpen, onClose, title, children, size = 'md' }: ModalProps) {
  ...
  return (
    ...
    >
      <div 
        className={`bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl w-full ${sizeClasses[size]} fade-in overflow-hidden`}
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h3 id="modal-title" className="text-xl font-bold text-primary dark:text-white font-mono tracking-tight">
              {title}
            </h3>
            <button
              onClick={onClose}
